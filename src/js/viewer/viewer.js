...
papaya.viewer.Viewer.prototype.updatePosition = function (viewer, xLoc, yLoc, crosshairsOnly) {
    ...
    viewer.screenVolumes[0].rotateLocalizer(0, this.currentInteractingSlice.sliceDirection, false); // update other viewport's image matrices
...
papaya.viewer.Viewer.prototype.drawViewer = function (force, skipUpdate, forceMIP, isRotating) {
    ...
    if (skipUpdate) {
        ...
        if (force || (this.draggingSliceDir !== papaya.viewer.ScreenSlice.DIRECTION_AXIAL)) {
            this.axialSlice.updateSlice(this.currentCoord.z, force, isRotating, this.worldSpace);
        }

        if (force || (this.draggingSliceDir !== papaya.viewer.ScreenSlice.DIRECTION_CORONAL)) {
            this.coronalSlice.updateSlice(this.currentCoord.y, force, isRotating, this.worldSpace);
        }

        if (force || (this.draggingSliceDir !== papaya.viewer.ScreenSlice.DIRECTION_SAGITTAL)) {
            this.sagittalSlice.updateSlice(this.currentCoord.x, force, isRotating, this.worldSpace);
        }
    }
...
papaya.viewer.Viewer.prototype.mouseMoveEvent = function (me) {
    ...
    if (this.isDragging) {
        ...
        if (this.grabbedHandle || this.activeTool === 'Ruler' && !this.isGrabbingLocalizer) {
        ...
        } else if (this.activeTool === 'Window' && !this.isGrabbingLocalizer) { // original: else if (this.isWindowControl)
        ...
        } else if (this.activeTool === 'StackScroll' && !this.isGrabbingLocalizer) {
        ...
        //////////////////////////////////////////////
        } else if (this.isPanning && !this.isGrabbingLocalizer) {
            if (this.selectedSlice === this.surfaceView) {
        ...
        } else if (this.isZoomMode && !this.isGrabbingLocalizer) {
            if (this.selectedSlice === this.surfaceView) {
            ...
            this.drawViewer(true);
        } else if (this.localizerDetected === 2) { // original: else, no if
        ...
        } else if (this.localizerDetected === 1) { // original: else, no if
            // console.log('mousemove ROTATE', localizerCenter);
            // this.resetUpdateTimer(null);
            var rotateAngle = this.getRotatingAngle(this.currentInteractingSlice.localizerCenter, this.previousMousePosition.x, this.previousMousePosition.y, currentMouseX, currentMouseY);
            this.screenVolumes[0].rotateLocalizer(rotateAngle, this.currentInteractingSlice.sliceDirection, true);
            this.previousMousePosition.x = currentMouseX;
            this.previousMousePosition.y = currentMouseY;
...
papaya.viewer.Viewer.prototype.getRotatingAngle = function (center, preX, preY, mouseX, mouseY) {
    this.updateOffsetRect();
